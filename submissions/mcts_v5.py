...
class MCTSAgent(Agent):
    ...
    def act(self, obs: Any, deadline: Deadline) -> Action:
        ...
        # Reset per-match state on match-start detection. Matches
        # HeuristicAgent.act(): seat 0 -> obs.step==0; seat 1 ->
        # next_fleet_id regression or first call (since Kaggle omits
        # step for player 1).
        raw_step = obs_get(obs, "step", None)
        curr_nfid = int(obs_get(obs, "next_fleet_id", 0))
        if raw_step is not None:
